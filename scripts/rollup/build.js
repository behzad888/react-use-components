...
function getPlugins(
  ...
  const isFBBundle =
    bundleType === RC_WWW_DEV ||
    bundleType === RC_WWW_PROD ||
    bundleType === RC_WWW_PROFILING;
  const isRNBundle =
    bundleType === RN_OSS_DEV ||
    bundleType === RN_OSS_PROD ||
    bundleType === RN_OSS_PROFILING ||
    bundleType === RN_RC_DEV ||
    bundleType === RN_RC_PROD ||
    bundleType === RN_RC_PROFILING;
...
async function buildEverything() {
  ...
  for (const bundle of Bundles.bundles) {
    ...
    await createBundle(bundle, RC_WWW_DEV);
    await createBundle(bundle, RC_WWW_PROD);
    await createBundle(bundle, RC_WWW_PROFILING);
    await createBundle(bundle, RN_OSS_DEV);
    await createBundle(bundle, RN_OSS_PROD);
    await createBundle(bundle, RN_OSS_PROFILING);
    await createBundle(bundle, RN_RC_DEV);
    await createBundle(bundle, RN_RC_PROD);
    await createBundle(bundle, RN_RC_PROFILING);
  }
